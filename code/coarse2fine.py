...
class C2F(torch.nn.Module):
    def __init__(self, len1, len2, len3, model):
        ...
        self.convs = torch.nn.ModuleList([
            torch.nn.Sequential(
                ...
                torch.nn.ReLU(),
                torch.nn.MaxPool2d(kernel_size=(self.height - h + 1, 1), stride=(self.height - h + 1, 1))) for h in
            [2, 3, 4]])
    ...
    def train(self, train_x1, train_y1, test_x1, test_y1, train_x2, train_y2, test_x2, test_y2, train_x3s, train_x3w,
              train_y3, test_x3s, test_x3w, test_y3):
        ...
        for epoch in range(100):
            ...
            batch_y1 = torch.autograd.Variable(
                torch.LongTensor(np.array([obj for i, obj in enumerate(train_y1) if i in rand_index_x1])))
            ...
            df1 = pd.concat([df1, pd.DataFrame(
                {'recall': [recall], 'precision': [precision], 'macrof1': [macrof1], 'microf1': [microf1],
                 'acc': [acc]})], axis=0, ignore_index=True)
            print(
                "[{:4d}]    recall:{:.4%}    precision:{:.4%}    macrof1:{:.4%}    microf1:{:.4%}    accuracy:{:.4%}".format(
                    epoch, recall, precision, macrof1, microf1, acc))
        ...
        for epoch in range(200):
            ...
            batch_x2 = torch.autograd.Variable(
                torch.Tensor(np.array([[obj] for i, obj in enumerate(train_x2) if i in rand_index_x2])))
            batch_y2 = torch.autograd.Variable(
                torch.LongTensor(np.array([obj for i, obj in enumerate(train_y2) if i in rand_index_x2])))
        ...
        for epoch in range(1000):
            ...
            batch_x3s = torch.autograd.Variable(
                torch.Tensor(np.array([[obj] for i, obj in enumerate(train_x3s) if i in rand_index_x3])))
            batch_x3w = torch.autograd.Variable(
                torch.Tensor(np.array([obj for i, obj in enumerate(train_x3w) if i in rand_index_x3])))
            batch_y3 = torch.autograd.Variable(
                torch.LongTensor(np.array([obj for i, obj in enumerate(train_y3) if i in rand_index_x3])))
